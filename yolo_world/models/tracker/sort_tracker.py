...
from ..utils import bbox_xyxy_to_cxcyah
...
class SortTracker(BaseTracker):
    ...
    def update_track(self, id, obj):
        ...
        assert bbox.ndim == 2 and bbox.shape[0] == 1
        bbox = bbox.squeeze(0).cpu().numpy()
        mean = self.tracks[id].mean
        covariance = self.tracks[id].covariance

        # Check for inf/nan in mean or covariance
        if np.any(np.isnan(mean)) or np.any(np.isinf(mean)) \
                or np.any(np.isnan(covariance)) or np.any(np.isinf(covariance)) \
                or np.any(np.diag(covariance) <= 0):
            print("Track", id, "is invalid. Resetting!")
            self.tracks[id].mean = np.zeros_like(mean)
            self.tracks[id].covariance = np.eye(mean.shape[0]) * 10.0

            # Optionally: reset the track here, or continue without updating
        else:
            try:
                self.tracks[id].mean, self.tracks[id].covariance = self.kf.update(
                    mean, covariance, bbox)
            except np.linalg.LinAlgError:
                print("Failed on mean:\n", mean)
                print("Failed on covariance:\n", covariance)
                print("Failed on bbox:\n", bbox)
                # Optionally: reset the track here, or continue without updating
                pass  # Do not re-raise, just skip the update
