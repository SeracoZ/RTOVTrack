        ...
        if method == "ovtrack-teta":
            ...
            if bboxes.size(0) > 0 and not self.empty:
                ...
                if self.match_metric == "bisoftmax":
                    if self.use_embed == "cls_embeds":
                        using_embed = embeds
                        using_memo_embed = memo_embeds
                    else:
                        using_embed = cls_embeds
                        using_memo_embed = memo_cls_embeds
                    sims = cal_similarity(
                        using_embed,
                        using_memo_embed,
                        method="dot_product",
                    ...
                    cos_scores = cal_similarity(using_embed, using_memo_embed, method="cosine")
