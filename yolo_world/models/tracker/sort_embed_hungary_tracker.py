...
class SortEmbedHungTracker(BaseTracker):
    ...
    def __init__(self,
        ...
        self.num_samples_reid = num_samples_reid
    ...
    def track(self,
        ...
        embeds = data_sample.pred_instances.embeds
        ...
        if self.empty or bboxes.size(0) == 0:
            ...
            # motion xiao: not use motion information here;
            ...
            # only rely on embed information;

            if len(active_ids) > 0:
                track_embeds = self.get(
                    'embeds',
                    active_ids,
                    self.num_samples_reid,
                    behavior='mean')

                #reid_dists = torch.cdist(track_embeds, embeds)

                #xiao: base on cosine similarity
                match_scores = torch.mm(
                    F.normalize(track_embeds, p=2, dim=1),
                    F.normalize(embeds, p=2, dim=1).t())

                reid_dists = 1 - match_scores
                ...
                #reid_dists[~np.isfinite(costs[valid_inds, :])] = np.nan  # xiao: not used iou similarity
