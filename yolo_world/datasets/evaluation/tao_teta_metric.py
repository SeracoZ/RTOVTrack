...
class TaoTETAMetric(BaseVideoMetric):
    ...
    def __init__(
        ...
        evalfile_prefix: Optional[str] = None,
        ...
        self.evalfile_prefix = evalfile_prefix
        ...
        assert dataset_type in ["Taov05Dataset", "Taov1Dataset", "Taov1_TrainDataset", "TaoTestDataset", "OVTBDataset"]
        if dataset_type == "Taov05Dataset":
            from yolo_world.datasets import Taov05Dataset
            self.class_list = Taov05Dataset.METAINFO["classes"]

        if dataset_type == "Taov1Dataset":
            from yolo_world.datasets import Taov1Dataset
            self.class_list = Taov1Dataset.METAINFO["classes"]

        if dataset_type == "Taov1_TrainDataset":
            from yolo_world.datasets import Taov1_TrainDataset
            self.class_list = Taov1_TrainDataset.METAINFO["classes"]

        if dataset_type == "TaoTestDataset":
            from yolo_world.datasets import TaoTestDataset
            self.class_list = TaoTestDataset.METAINFO["classes"]

        if dataset_type == "OVTBDataset":
            from yolo_world.datasets import OVTBDataset
            self.class_list = OVTBDataset.METAINFO["classes"]


        self.cat_ids = self.coco.get_cat_ids(cat_names=self.class_list)

    # xiao: commented out this
    # def __del__(self):
    ...
    def compute_metrics(self, results: list = None) -> dict:
        ...
        if self.evalfile_prefix is not None:
            resfile_path = self.evalfile_prefix
        else:
        ...
        default_eval_config["NUM_PARALLEL_CORES"] = 4
    ...
    def remap_seq_pred_image_ids(self, remap_dict: dict):
        """Remap image_id in self.seq_pred using remap_dict and remove unmatched entries."""
        new_seq_pred = defaultdict(list)

        for video_id, preds in self.seq_pred.items():
            for pred in preds:
                new_id = pred["image_id"]
                if new_id in remap_dict:
                    new_pred = pred.copy()
                    new_pred["image_id"] = remap_dict[new_id]
                    new_seq_pred[video_id].append(new_pred)


        self.seq_pred = new_seq_pred
    ...
    def evaluate(self, size: int = 1) -> dict:
        ...
        if hasattr(self, "remap_file") and self.remap_file is not None:
            with open(self.remap_file) as f:
                mapping = json.load(f)
            remap_dict = {entry["new_image_id"]: entry["old_image_id"] for entry in mapping}
            self.remap_seq_pred_image_ids(remap_dict)
