...
from yolo_world.models.utils.list_LVIS import CLASSES

print("[DEBUG] Registered LVIS_seqs_Dataset from", __file__)
...
class LVIS_seqs_Dataset(TaoDataset):
    def __init__(self,
        ...
        **kwargs,):
        ...
        # ==== REMOVE unwanted custom keys so parent doesn't see them ====
        kwargs.pop('args_mot', None)
        kwargs.pop('train_pipeline', None)
        kwargs.pop('classes', None)  # If parent doesn't use it

        super(LVIS_seqs_Dataset, self).__init__(
            ann_file=ann_file,
            pipeline=pipeline,
            load_as_video=True,
            match_gts=True,
        ...
        # MMYOLO/MMDet COMPATIBILITY: force loading and filtering!
        self.data_infos = self.load_annotations(self.ann_file)
        self._filter_imgs()  # if you want to filter invalid images
    ...
    def __getitem__(self, idx_sampled):
        ...
        # Only print if idx is problematic:
        if not (0 <= idx < len(self.data_infos)):
            print("="*40)
            print(f"[ERROR] __getitem__: idx_sampled={idx_sampled}, resolved idx={idx}, len(data_infos)={len(self.data_infos)}")
            ...
            for (transform, transform_type) in zip(self.pipeline_seq,
                ...
                if hasattr(transform, 'get_indexes'):
                    ...
                    if random_value < 0.5:  
                        ...
                        mix_results = []
                        instance_num = len(results[0]['gt_bboxes_labels'])
                        for index in indexes:
                            ...
                            instance_num += len(results_i[0]['gt_bboxes_labels'])
                ...
                elif transform_type == 'SeqPad':
                    results = transform(results)
                    img_shape = results[0]['img_shape']
                    ori_shape = results[0]['ori_shape']
                    #print(f'[SeqPad Debug] ori_shape={ori_shape}, img_shape={img_shape}')
                    #if img_shape[0] > 800 or img_shape[1] > 1344:
                        #print(f'[ERROR] After SeqResize: img_shape exceeds target! img_shape={img_shape}, ori_shape={ori_shape}')
                else:    
                    results = transform(results)
...
########################
            try:
                ...
                print(f"Exception in processing idx: {idx}")
                import traceback
                traceback.print_exc()
                print(f"results: {results if 'results' in locals() else None}")
                # You can print other intermediate variables as needed
                raise
################################################
            '''
            if isinstance(results, dict): # Check if the object is empty.
    ...
    def category_unique(self, results):
        ...
        for _result in results:
            labels = _result['gt_bboxes_labels']
    ...
    def get_min_wh(self, results):
        gt_bboxes = results[0]['gt_bboxes']
        # If MMDet Box class, use .tensor
        if hasattr(gt_bboxes, 'tensor'):
            gt_bboxes = gt_bboxes.tensor
        # Now gt_bboxes is either a torch tensor or numpy array
        if len(gt_bboxes) == 0:
            return [0, 0]
        width = (gt_bboxes[:, 2] - gt_bboxes[:, 0])
        height = (gt_bboxes[:, 3] - gt_bboxes[:, 1])
        # If using torch tensors, get scalar value with .item()
        min_width = width.min().item() if hasattr(width, 'min') else width.min()
        min_height = height.min().item() if hasattr(height, 'min') else height.min()
        return [min_width, min_height]

    def get_area_ratio(self, results):
        area_ratio = []
        for _result in results:
            gt_bboxes = _result['gt_bboxes']
            # FIX: Always get the tensor if HorizontalBoxes or MMDet box class
            if hasattr(gt_bboxes, 'tensor'):
                gt_bboxes = gt_bboxes.tensor
            if len(gt_bboxes) == 0:
                area_ratio.append(torch.tensor([]))  # or np.array([]), as you need
                continue
            _area_ratio = (gt_bboxes[:, 2] - gt_bboxes[:, 0]) * (gt_bboxes[:, 3] - gt_bboxes[:, 1])
    ...
    def visualize(self, results):
        ...
        for i, _result in enumerate(results):
            ...
            for x, label in zip(bboxs, _result['gt_bboxes_labels']):
                if hasattr(x, 'tensor'):
                    box = x.tensor[0]
                else:
                    # fallback, maybe it's already a tensor/array
                    box = x
    ...
    def last_visualize(self, results):
        Path('./data_vis').mkdir(parents=True, exist_ok=True)
        name = None
        root = str(Path.cwd())
        for i, _result in enumerate(results):
    ...
    def results_to_instances(self, results):
        ...
        results_seqs = []
        for i in range(self.num_frames_per_batch):
            results_seqs.append({'img_metas':results[f'img_meta_{i}'],
                                 'img':results[f'img_{i}'],
                                 'gt_bboxes':results[f'gt_bboxes_{i}'],
                                 'gt_bboxes_labels':results[f'gt_bboxes_labels_{i}'],
                                 'gt_match_indices':results[f'gt_match_indices_{i}'],
                                 'img_info':results[f'img_info_{i}'],
                                 'ann_info':results[f'ann_info_{i}'],
                                })
    ...
    def _target_to_instances(self, data):
        img_shape = data['img_metas'].data['pad_shape']
        return {
            'bboxes': data['gt_bboxes'].data,  # Or convert to cxcywh if needed
            'labels': data['gt_bboxes_labels'],
            'instance_ids': data['gt_match_indices'],
    ...
    def prepare_train_img(self, idx):
        ...
        """
        if not (0 <= idx < len(self.data_infos)):
            print(f"[WARN] idx={idx} out of range (0, {len(self.data_infos) - 1}), skipping...")
            return None
